use chrono::Utc;
use egui::{pos2, vec2, Align2, Color32, FontId, InnerResponse, Layout, Rect, Rounding, TextEdit, TextStyle, Ui};

use crate::{backend::{profile::BlueskyApiProfile, record::{BlueskyApiRecord, BlueskyApiRecordPost, BlueskyApiReplyRef}}, bridge::Bridge, frontend::{circle_button, main::{ClientFrontend, ClientFrontendFlyoutVariant}}, image::ImageCache, widgets::spinner::SegoeBootSpinner};
...
fn render_mini_profile(ui: &mut Ui,
    image: &ImageCache,
    avatar: &Option<String>,
    display_name: &Option<String>,
    handle: &String) -> egui::Response {
    ...
    let tex = if let Some(avatar) = &avatar {
        match image.get_image(&avatar) {
            crate::image::LoadableImage::Unloaded |
            crate::image::LoadableImage::Loading => None,
...
impl ClientFrontendFlyoutVariant {
    pub fn post_composer(ui: &mut Ui,
        data: &mut ComposerFlyout,
        profile: &Option<BlueskyApiProfile>,
        ...
        ui.add_enabled_ui(!data.sending, |ui| {
            ...
            ui.with_layout(Layout::left_to_right(egui::Align::Min), |ui| {
                let res = if let Some(profile) = &profile {
                    render_mini_profile(ui, img_cache, &profile.avatar, &profile.display_name, &profile.handle)
                } else {
            ...
            ui.with_layout(Layout::left_to_right(egui::Align::Min), |buttons| {
                ...
                if send_button.clicked() {
                    ...
                    let record = BlueskyApiRecord::Post(BlueskyApiRecordPost {
                        text: data.draft.clone(),
                        created_at: Utc::now(),
                        facets: None,
                        reply: data.reply.clone(),
                        embed: None,
                        langs: Some(languages),
                        labels: None,
                        tags: None,
                    });
