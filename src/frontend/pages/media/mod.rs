...
impl FrontendMediaViewVariant {
    pub fn render(&mut self, ui: &mut egui::Ui, image: &ImageCache, _new_view: &mut MainViewProposition) -> ViewStackReturnInfo {
        ...
        let back_button = ui.allocate_rect(egui::Rect::from_center_size(ui.ctx().screen_rect().left_top() + vec2(60.0, 60.0), vec2(40.0, 40.0)), egui::Sense::click()).on_hover_cursor(egui::CursorIcon::PointingHand);        
        ui.painter().text(back_button.rect.center(), Align2::CENTER_CENTER, "\u{E0BA}", FontId::new(40.0, egui::FontFamily::Name("Segoe Symbols".into())), Color32::WHITE);
        ViewStackReturnInfo {
            title: None,
            render_back_button: false,
            handle_back_logic: true,
            force_back: back_button.clicked(),
        }
    }
}
