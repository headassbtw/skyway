...
impl FrontendProfileView {
    ...
    pub fn render(&mut self, ui: &mut Ui, modal: &mut ClientFrontendModal, backend: &Bridge, image: &ImageCache, flyout: &mut ClientFrontendFlyout, new_view: &mut MainViewProposition) -> ViewStackReturnInfo {
        ...
        if let Some(profile) = &self.profile_data {
            ...
            let mut who_gaf = ui.new_child(UiBuilder::new().layout(Layout::left_to_right(egui::Align::Min)).max_rect(funny_rect));
            ...
            self.id = ScrollArea::horizontal()
                .id_salt(format!("{}_profile_scrollview", profile.did))
                .vscroll(false)
                .max_width(funny_rect.width())
                .max_height(funny_rect.height())
                ...
                .show(&mut who_gaf, |ui| {
                    ...
                    if scroll_to_posts {
                        posts_res.response.scroll_to_me(Some(Align::TOP));
                    }
